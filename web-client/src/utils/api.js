...
export const submitMovementAWord = (t, gameId, word) =>
  req(`/api/games/${gameId}/movement-a/submit`, { method: 'POST', headers: { ...jsonH, ...authH(t), ...ngrokH }, body: JSON.stringify({ word }) })

export const submitMovementASketch = (t, gameId, sketchData) =>
  req(`/api/games/${gameId}/movement-a/submit`, { method: 'POST', headers: { ...jsonH, ...authH(t), ...ngrokH }, body: JSON.stringify({ sketchData: JSON.stringify(sketchData) }) })
